...
namespace MyApp1.Application.Services
{
    ...
    public class TokenService : ITokenService
    {
        ...
        public (string Token, string RefreshToken) GenerateTokens(User user)
        {
            var jwtToken = JwtHelper.GenerateJwtToken(user, _configuration);
            var refreshToken = CreateRefreshToken();
            ...
            var refreshTokenEntity = new RefreshToken
            {
                ...
                Expires = DateTime.UtcNow.AddMinutes(Convert.ToDouble(_configuration["JwtSettings:RefreshTokenExpiryMinutes"])),
                UserId = user.Id
            };
        ...
        public async Task<(string Token, string RefreshToken)> RefreshTokenAsync(string token, string refreshToken)
        {
            ...
            var storedToken = _context.RefreshTokens
                .SingleOrDefault(rt => rt.Token == refreshToken && rt.UserId == userId);
        ...
        public async Task RevokeRefreshToken(string refreshToken)
        {
            var stored = _context.RefreshTokens.SingleOrDefault(rt => rt.Token == refreshToken);
            if (stored != null)
            {
